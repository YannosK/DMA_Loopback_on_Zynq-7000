...
#define TEST_START_VALUE (INT32_MAX - 1) // test rollover overflow - max int32 value should be 2^31 - 1 (signed)
 ...
 *
 * @param   Buff is the buffer where the DMA stored the data (pointer to first element)
...
static int CheckData(int32_t *Buff, int Length, int32_t StartValue)
{
     ...
     */
    Xil_DCacheInvalidateRange((UINTPTR)Buff, Length * sizeof(int32_t));
